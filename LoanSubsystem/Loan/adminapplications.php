...
// Joins all normalized loandb tables to reconstruct the full application row.
//
// Tables used:
//   loan_applications  → core loan data (user_id, loan_type_id, user_email,
//                         loan_terms, loan_amount, monthly_payment,
//                         next_payment_due, purpose, status, created_at)
//   loan_types         → loan type name        (via loan_type_id)
//   loan_borrowers     → full_name, account_number, contact_number,
//                         email, job, monthly_salary
//   loan_valid_ids     → loan_valid_id_type FK + valid_id_number
//   loan_valid_id      → valid_id_type label   (via loan_valid_ids.loan_valid_id_type)
//   loan_approvals     → approved_at, approved_by, approved_by_user_id
//   loan_rejections    → rejected_at, rejected_by, rejection_remarks
//   loan_documents     → file_name, proof_of_income, coe_document
// ─────────────────────────────────────────────────────────────────────────────
...
<main>
  ...
  <!-- ── APPROVED TABLE (awaiting claim / 2nd approval) ─────────────────────── -->
  ...
  <div class="table-container">
    <table>
      ...
      <tbody id="approvedTableBody">
        <?php
        // Order by lap.approved_at since approval timestamp is in loan_approvals
        ...
        if ($result && $result->num_rows > 0):
            while ($row = $result->fetch_assoc()):
                $approved_date = $row['approved_at']
                    ...
                    : '—';
                $approved_time = $row['approved_at']
                    ? date("h:i A", strtotime($row['approved_at']))
                    : '';
        ?>
            <tr data-loan-id="<?= (int)$row['id'] ?>">
              <td><?= htmlspecialchars($row['id']) ?></td>
...
<script>
  ...
  function viewLoanApplication(loanId, stage) {
    ...
    fetch('view_loan.php?id=' + loanId)
      ...
      .then(data => {
        ...
        approveBtn.textContent = stage === 'approved' ? 'Activate Loan' : 'Approve';
  ...
  function updateLoanStatus(loanId, status, action, remarks) {
    ...
    .then(data => {
      if (data.success) {
        // ✅ Always show the success message — never block on credit_status
        alert(data.message);
        ...
        // Only real DB/auth failures land here
